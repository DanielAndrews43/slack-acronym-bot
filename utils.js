...
const NO_DEFINITION_ERROR_MESSAGE = (acronym) =>
  `Sorry, _${acronym}_ has not been defined yet!\nYou can define a new acronym using _/explain ${acronym} definition_`;
const THANK_YOU_MESSAGE = (acronym) =>
  `Thanks for your definition of _${acronym}_!`;
const NEEDS_DEFINITION_MESSAGE = (acronym, username) =>
  `${username} needs to define *_${acronym}_*!\nYou can define a new acronym using _/explain ${acronym} definition_`;
const WAS_DEFINED_MESSAGE = (acronym, username, definition) =>
  `${username} just defined _${acronym}_ as\n>${definition}`;
const ACRONYM_DELETED_MESSAGE = (acronym) =>
  `You have successfully removed the definition(s) for _${acronym}_`;
const LISTALL_MESSAGE = (message) =>
  `This is a list of all definitions for your Slack workspace. Copy and paste it into your next workspace to import the data.\n\n${message}`;
const DEFINITION_MESSAGE = (acronym, definitions) => {
  ...
  try {
    ...
    return "Sorry, there was an error getting the definition for " + acronym;
  }
...
const postDefinition = (
  ...
  if (definitions?.length) {
    ...
    postToChannel(
      NEEDS_DEFINITION_MESSAGE(acronym, username),
      channel_id,
      token
    );
...
const newDefinition = (
  ...
  postToChannel(
    WAS_DEFINED_MESSAGE(acronym, username, definition),
    channel_id,
    token
  );
...
exports.saveToken = (params) => {
  saveTeamInfo(
    params.team_id,
    params.incoming_webhook.channel_id,
    params.access_token
  );
